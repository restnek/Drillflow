...
public class ChannelSetConversionTest {
  ...
  public void shouldCovertChannelSetFrom1411() throws JAXBException, IOException {
    ...
    assertEquals(log.getCreationDate(), channelSet.getCitation().getCreation());
    ...
    assertEquals(
        log.getStepIncrement().getValue().toString(), channelSet.getStepIncrement().getValue());
    assertEquals(log.getNullValue(), channelSet.getNullValue());
    for (int i = 0; i < log.getLogParam().size(); i++) {
      assertEquals(
          log.getLogParam().get(i).getDescription(),
          channelSet.getLogParam().get(i).getDescription());
      assertEquals(log.getLogParam().get(i).getIndex(), channelSet.getLogParam().get(i).getIndex());
      assertEquals(log.getLogParam().get(i).getName(), channelSet.getLogParam().get(i).getName());
      assertEquals(log.getLogParam().get(i).getUom(), channelSet.getLogParam().get(i).getUom());
      assertEquals(log.getLogParam().get(i).getUid(), channelSet.getLogParam().get(i).getUid());
      assertEquals(log.getLogParam().get(i).getValue(), channelSet.getLogParam().get(i).getValue());
    }
  ...
  public void shouldCovertTimeChannelSetFrom1411() throws JAXBException, IOException {
    ...
    assertEquals(log.getCreationDate(), channelSet.getCitation().getCreation());
    assertEquals(log.getDescription(), channelSet.getCitation().getDescription());
    assertEquals(log.getStartDateTimeIndex(), channelSet.getStartIndex());
    assertEquals(log.getEndDateTimeIndex(), channelSet.getEndIndex());
    assertEquals(log.getStepIncrement().getUom(), channelSet.getStepIncrement().getUom());
    assertEquals(
        log.getStepIncrement().getValue().toString(), channelSet.getStepIncrement().getValue());
    assertEquals(log.getNullValue(), channelSet.getNullValue());
