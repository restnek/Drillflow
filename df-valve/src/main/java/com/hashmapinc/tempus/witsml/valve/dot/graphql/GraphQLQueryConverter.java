...
public class GraphQLQueryConverter {
  ...
  /**
   * this function converts 2.0 trajectory json into a graphql query "select *"-style query
   *
  ...
  private static String getTrajectoryQuery(AbstractWitsmlObject wmlObject, String wbUuid) {
    ...
    // parse json strings
    String jsonString1411 =
        wmlObject.getJSONString(
            "1.4.1.1"); // needed for some query args. some fields are lost in 2.0
    ...
    // nameWellbore
    if (objTrajectoryJson.has("nameWellbore")
        && !JsonUtil.isEmpty(objTrajectoryJson.get("nameWellbore")))
      trajQueryFields.put("nameWellbore", objTrajectoryJson.get("nameWellbore"));

    // growingStatus
    if (trajectoryJson.has("growingStatus")
        && !JsonUtil.isEmpty(trajectoryJson.get("growingStatus")))
  ...
  /**
   * This function accepts any wmlObject that has a wellbore parent and a well grandparent and
   * builds a graphql query to get the uuid's of the well/wellbore.
   *
  ...
  /**
   * This function accepts any wmlObject that has a well parent and a well grandparent and builds a
   * graphql query to get the uuid's of the well/wellbore.
   *
