 ...
 *
 * <p>Search in the cache is never granular; it is against the entire channelSet.
 */
public class ChannelSetCache {
  ...
  /**
   * This function stores a ChannelSet (JSON format) in cache along with the composite key of uuid +
   * SEPARATOR + hashCode() (from the ChannelSet).
   *
   * @param uuid - string uuid used as a cache key
   * @param cs - query's ChannelSet converted for DoT's API (v1.4.1.1 or v1.3.1.1)
   */
  public static void putInCache(String uuid, ChannelSet cs) throws JsonProcessingException {
  ...
  /**
   * This function returns the matching ChannelSet from cache to the one passed in.
   *
   ...
   */
  public static String getCS(String uuid, ChannelSet cs) {
    // check cache
