 ...
 * uuid->uid.
 *
 * <p>There are 3 types of objects supported in this map: 1: parentless objects with no parent uid
 ...
 *
 * <p>The composite keys for each supported object case is: 1: parentless objects use "object_uid"
 * as the key 2: single parent objects use "parent_uid|===|object_uid" as the key 3: single
 * grandparent objects use "grandparent_uid|===|parent_uid|===|object_uid" as the key
 *
 * <p>each composite key points directly to a UUID string.
 *
...
public class UidUuidCache {
   ...
   */
  public static void putInCache(String uuid, String uid) {
   ...
   * @param parentUid - string uid of the parent object needed for building the composite key
   */
  public static void putInCache(String uuid, String uid, String parentUid) {
    // build composite key
   ...
   * @param uid - string uid to cache
   * @param parentUid - string uid of the parent object needed for building the composite key
   * @param grandparentUid - string uid of the grandparent object needed for building the composite
   *     key
   */
  public static void putInCache(String uuid, String uid, String parentUid, String grandparentUid) {
    // build composite key
   ...
   *
   * <p>If a match is found, it is returned.
   *
  ...
  /**
   * returns the uuid for a grandparent-only object with the given uid and parentUid.
   *
   * @param uid - string identifier for the object uuid being requested
  ...
  public static String getUuid(String uid, String parentUid) {
    ...
    // check cache
    if (cache.containsKey(compositeKey)) return cache.get(compositeKey);

    return null; // no match found
  }

  /**
   * returns the uuid for a grandparent-only object with the given uid, parentUid, and
   * grandparentUid.
   *
   * @param uid - string identifier for the object uuid being requested
   * @param parentUid - string uid of the parent of the object UUID being requested
