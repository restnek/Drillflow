...
public class StoreValidator {
   ...
   * @return 1 if it's all good, other if it's not.
   */
  public static short validateGetCap(String optionsIn) {
    Map<String, String> options = WitsmlUtil.parseOptionsIn(optionsIn);
    if (!optionsIn.isEmpty() && options.size() == 0) return -411;
    if (!options.containsKey("dataVersion")) return -424;
    return 1;
  }
   ...
   * @return The return code, 1 if it's all good and can proceed, not one if there is an issue that should be reported
   */
  public static short validateAddToStore(String wmlTypeIn, String xmlIn, IValve valve) {
    if (wmlTypeIn.isEmpty()) return -407;
    if (xmlIn.isEmpty()) return -408;
    if (!isTypeMatch(wmlTypeIn, xmlIn)) return -486;
    if (!isObjectSupported(wmlTypeIn, valve, "WMLS_AddToStore")) return -487;
    if (!containsPluralRoot(wmlTypeIn, xmlIn)) return -401;
    if (!containsVersion(xmlIn)) return -468;
    if (!containsDefaultNamespace(xmlIn)) return -403;
    return 1;
  }
   ...
   */
  public static short validateGetFromStore(
      String wmlTypeIn, String xmlIn, Map<String, String> optionsMap, IValve valve) {
    ...
    if (optionsMap.containsKey("requestObjectSelectionCapability")) {
      ...
      if (keyValue.equals("true")) {
        ...
        if (optionsMap.size() > 2) return -427;
  ...
  private static short standardValidations(String wmlTypeIn, String xmlIn, IValve valve) {
    ...
    if (wmlTypeIn.isBlank()) return -407;
    if (!isObjectSupported(wmlTypeIn, valve, "WMLS_GetFromStore")) return -487;
    if (xmlIn.isEmpty()) return -408;
    if (!containsVersion(xmlIn)) return -468;
    return 1;
  }
   ...
   * @return The return code, 1 if it's all good and can proceed, not one if there is an issue that should be reported
   */
  public static short validateUpdateInStore(
   ...
   * @return The return code, 1 if it's all good and can proceed, not one if there is an issue that should be reported
   */
  public static short validateDeleteFromStore(
      String wmlTypeIn, String xmlIn, Map<String, String> optionsIn, IValve valve) {
    return 1;
  }
   ...
   * @return true if the object is supported, false if it's not
   */
  private static boolean isObjectSupported(String wmlTypeIn, IValve valve, String operation) {
    if (!valve.getCap().containsKey(operation)) return false;
    AbstractWitsmlObject[] supportedObjs = valve.getCap().get(operation);
    if (supportedObjs == null) return false;
    return Stream.of(supportedObjs)
        .map(AbstractWitsmlObject::getObjectType)
        .anyMatch(wmlTypeIn.toLowerCase()::equals);
  }
  ...
  private static boolean containsVersion(String xmlIn) {
    try {
      WitsmlUtil.getVersionFromXML(xmlIn);
      return true;
    } catch (Exception e) {
      return false;
    }
